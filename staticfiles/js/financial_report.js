...
class FinancialReportManager {
    ...
    renderReport(data) {
        ...
        this.renderChart();
        this.renderDonutCharts(data);  // NEW: Add donut charts
    ...
    renderDonutCharts(data) {
        this.renderIncomeDonut(data.income);
        this.renderExpenseDonut(data.expenses);
    }

    renderIncomeDonut(income) {
        const canvas = document.getElementById('incomeDonutChart');
        if (!canvas) return;

        if (this.incomeDonutChart) {
            this.incomeDonutChart.destroy();
        }

        const labels = [];
        const values = [];
        const colors = ['rgb(16, 185, 129)', 'rgb(59, 130, 246)', 'rgb(168, 85, 247)'];

        if (income.payments.total > 0) {
            labels.push('اشتراكات الأعضاء');
            values.push(income.payments.total);
        }
        if (income.articles.total > 0) {
            labels.push('الأنشطة والدورات');
            values.push(income.articles.total);
        }
        if (income.added_payments.total > 0) {
            labels.push('إيرادات إضافية');
            values.push(income.added_payments.total);
        }
        ...
        const ctx = canvas.getContext('2d');
        this.incomeDonutChart = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels,
                datasets: [{
        ...
        this.renderCustomLegend('income-legend', labels, values, colors.slice(0, values.length));
    }
    ...
    renderExpenseDonut(expenses) {
        ...
        const labels = [];
        const values = [];
        const colors = ['rgb(239, 68, 68)', 'rgb(251, 146, 60)', 'rgb(245, 158, 11)', 'rgb(234, 179, 8)'];

        if (expenses.rent.total > 0) {
            labels.push('الإيجار');
            values.push(expenses.rent.total);
        }
        if (expenses.staff.total > 0) {
            labels.push('رواتب الموظفين');
            values.push(expenses.staff.total);
        }
        if (expenses.articles.total > 0) {
            labels.push('تكاليف الأنشطة');
            values.push(expenses.articles.total);
        }
        if (expenses.costs.total > 0) {
            labels.push('مصروفات أخرى');
            values.push(expenses.costs.total);
        }
        ...
        this.renderCustomLegend('expense-legend', labels, values, colors.slice(0, values.length));
    }

    renderCustomLegend(elementId, labels, values, colors) {
            ...
            html += `
                <div class="col-6">
                    <div class="d-flex align-items-center">
                        <div class="legend-color" style="background-color: ${color}; width: 12px; height: 12px; border-radius: 2px; margin-left: 8px;"></div>
                        <div class="flex-grow-1">
                            <div class="small fw-semibold">${label}</div>
                            <div class="small text-muted">${this.formatCurrency(value)} (${percentage}%)</div>
                        </div>
                    </div>
                </div>
            `;
